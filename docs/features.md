...
- `Raw JSON`, regex search, and outline navigation are intentionally disabled in `LARGE_PREVIEW`; the main screen labels the mode as a bounded preview, `Copy tree` becomes `Copy preview`, and warning icons mark large-preview inspections in the active file summary and both history lists.
