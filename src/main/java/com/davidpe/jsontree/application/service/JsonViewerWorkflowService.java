...
public class JsonViewerWorkflowService implements ImportJsonUseCase, OpenHistoryUseCase {
    ...
    public JsonViewerLoadResult loadFile(Path jsonFilePath) {
        ...
        if (!importResult.available()) {
            JsonViewerLoadResult unavailableResult = new JsonViewerLoadResult(
                    importResult,
                    new JsonValidationResult(JsonValidationStatus.PARSING_ERROR, "JSON file is not available.", null, null),
                    null,
                    null
            );
        ...
        JsonViewerLoadResult loadResult = new JsonViewerLoadResult(importResult, validationResult, asciiTreeDocument, historyEntry);
    ...
    public Optional<JsonViewerLoadResult> reopenHistoryEntry(String storedName) {
        ...
        JsonImportResult importResult = new JsonImportResult(
                virtualPath,
        ...
        JsonValidationResult validationResult = new JsonValidationResult(JsonValidationStatus.VALID, "Valid JSON.", null, null);
        AsciiTreeDocument asciiTreeDocument = asciiTreeRendererPort.render(writeTempSnapshot(storedJson.get(), storedName));
        JsonViewerLoadResult loadResult = new JsonViewerLoadResult(importResult, validationResult, asciiTreeDocument, historyEntry.get());
        currentView = loadResult;
    ...
    private ImportedJsonFile createHistoryEntry(JsonImportResult importResult, AsciiTreeDocument asciiTreeDocument) {
    ...
    private Path writeTempSnapshot(String content, String storedName) {
        try {
            ...
            Path tempFile = Files.createTempFile(FileSystems.getDefault().getPath(System.getProperty("java.io.tmpdir")), "json-tree-history-", suffix);
            Files.writeString(tempFile, content);
            tempFile.toFile().deleteOnExit();
            return tempFile;
        } catch (IOException exception) {
            throw new IllegalStateException("Unable to prepare stored JSON snapshot: " + storedName, exception);
        }
    }
}
